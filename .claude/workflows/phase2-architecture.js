...
const sad = await abLoop({
  ...
  buildAPrompt: (round, prevB2) =>
    ...
    + '   - §1 Overview. §2 Module design: every FR (FR-01..FR-05) maps to ≥1 module; follow SPEC.md §6 directory structure (src/taskq/{config,models,store,executor,breaker,cache,cli}.py). ≤15 files/dir, no god-module.\n'
    + '   - §3 Interfaces & data flows (consistent diagrams). §4 NFR handling (latency/security/reliability per SRS NFR-01..NFR-06).\n'
...
const testSpec = await abLoop({
  ...
  buildAPrompt: (round, prevB2) =>
    ...
    + '   - Per FR (FR-01..FR-05): assign Classification (API_ENDPOINT|DATA_ENTITY|ALGORITHM|STATE_MACHINE|INTEGRATION|SECURITY_CONTROL|INFRASTRUCTURE). ≥1 named test case (happy_path + validation mandatory). Preserve TEST_INVENTORY.yaml names where specified.\n'
...
const sabReport = await agent(
  ...
  + '   - nfr_traceability: each NFR-01..NFR-06 with a `type` from the 8 legal values (performance/security/maintainability/reliability/testability/deployability/scalability/usability) + measurable `target` + `module`.\n'
  + '   - fr_module_traceability: one entry per FR-01..FR-05 pointing to a REAL module from SAD §2.\n'
