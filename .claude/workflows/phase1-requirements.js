...
for (let round = 1; round <= MAX_B_ROUNDS; round++) {
  ...
  const aPromptHeader =
    ...
    + '   - 100% transcribe FR-01..FR-05 + NFR-01..NFR-06 from SPEC.md (read ' + REPO + '/SPEC.md). No invention, no omission.\n'
...
for (let round = 1; round <= MAX_B_ROUNDS; round++) {
  ...
  const aPromptHeader =
    ...
    + '2. Author SPEC_TRACKING.md as a markdown table. 11 rows (FR-01..FR-05 + NFR-01..NFR-06). Columns: FR ID | Description | Intent Class | Decision Framework | Status | Notes.\n'
...
for (let round = 1; round <= MAX_B_ROUNDS; round++) {
  ...
  const aPromptHeader =
    ...
    + '2. Author TEST_INVENTORY.yaml mapping every FR/NFR acceptance criterion to test function names. Schema: top-level `fr_tests:` map; each FR has `unit:` and `integration:` arrays of test function names. Naming: test_<module>_<scenario>. Cover FR-01..FR-05 + NFR-01..NFR-06. Each FR has >=3 unit + >=1 integration test. Header: `format_version: "1.1"`.\n'
    + '3. (Re-)read file via Read for final state.\n'
    + '4. If round > 1: apply HIGH-severity gap fixes from previous B-2 via Edit (surgical).\n'
    + '5. (Re-)read file for final state.\n'
