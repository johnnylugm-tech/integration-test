...
from taskq import cache, executor  # `cache` not yet created → RED Collection Error
...
def test_fr04_expiry_normal_execution(home):
    ...
    # AC-FR04-03-expiry-reruns: a normal (non-cached) execution after expiry
    # must still produce status="done".
    # GREEN TODO: on expiry, executor.execute runs the real subprocess and
    # returns status="done"; after the run, cache.put writes a fresh entry.
    result = executor.execute(command=command, timeout=10.0)
    if expected_status == "done":
        assert result.status == "done"
        assert expected_status == "done"
    if expected_cached == "false":
        # AC-FR04-03-cached-false: normal execution must NOT advertise cached=True.
